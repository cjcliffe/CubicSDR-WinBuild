...
if (-not ($AIRSPYONE_HOST_TARGET | Test-Path)) {
    ...
    cmake -B $AIRSPYONE_HOST_TARGET -G $CS_GENERATOR -A x64 $AIRSPYONE_HOST_SOURCES `
...
if (-not ($SOAPYAIRSPY_TARGET | Test-Path)) {
    ...
    cmake -B $SOAPYAIRSPY_TARGET -G $CS_GENERATOR -A x64 $SOAPYAIRSPY_SOURCES `
    ...
    cmake --build $SOAPYAIRSPY_TARGET --config Release --target install
}
