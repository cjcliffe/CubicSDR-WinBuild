...
if not exist hackrf (
    mkdir hackrf
    cmake -B hackrf_host -G %CS_GENERATOR% -A x64 %CS_SOURCES%/hackrf/host -DFFTW_LIBRARIES=%FFTW3_LIBRARIES% -DFFTW_INCLUDES=%CS_DEPS%/fftw-3.3.5-dll64 -DTHREADS_PTHREADS_WIN32_LIBRARY:PATH=%PTHREADS_LIBRARIES% -DTHREADS_PTHREADS_INCLUDE_DIR:PATH=%PTHREADS_INCLUDE_DIR% -DLIBUSB_INCLUDE_DIR:PATH=%LIBUSB_INCLUDE_DIR% -DLIBUSB_LIBRARIES:PATH=%LIBUSB_LIBRARIES% -DCMAKE_INSTALL_PREFIX=%CS_INSTALL%/hackrf
