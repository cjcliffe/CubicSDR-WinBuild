...
if (-not ($RTLSDR_TARGET | Test-Path)) {
    ...
    cmake -B $RTLSDR_TARGET -G $CS_GENERATOR -A x64 $RTLSDR_SOURCES `
...
if (-not ($SOAPY_RTLSDR_TARGET | Test-Path)) {
    ...
    cmake -B $SOAPY_RTLSDR_TARGET -G $CS_GENERATOR -A x64 $SOAPY_RTLSDR_SOURCES `
    ...
    cmake --build $SOAPY_RTLSDR_TARGET --config Release --target install
}
